...
int getOffsetGain(struct Matrix result, struct ellipsoid value) {  // result[9,1] A[4, 4]
    double g = 1 + (result.entry[6]*result.entry[6] / result.entry[0]
                + result.entry[7]*result.entry[7] / result.entry[1]
                + result.entry[8]*result.entry[8] / result.entry[2]);
    for(int i = 0; i < 3; i++) {
        value.offset[i] = result.entry[6+i] / result.entry[0 + i];
        value.gain[i] = g / result.entry[i];
    }
    return 0;
}
/* main API */
